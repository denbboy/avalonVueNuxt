<template>
  <section class="bg-blue-500 pt-32 md:pt-64 relative">
    ...
    <div
      class="bg-gradient-to-t from-blue-500 top-[calc(1000px_-_800px)] from-60% w-full h-[1000px] absolute z-10"
    ></div>
    <div
      class="bg-gradient-to-t from-transparent to-blue-500 w-full h-[400px] to-80% absolute top-0 z-10"
    ></div>
      ...
      <div class="pb-14 text-content">
        ...
        >
          <span class="text-white text-sm">
            {{ new Date(itemData?.date_created).getDate() }}
            {{ $t(`month${new Date(itemData?.date_created).getMonth() ?? '0' + 1}`) }}
      ...
      <div class="">
        <div class="flex justify-between items-center mb-8">
          ...
          <div class="w-fit hidden md:flex">
            ...
            <button
              type="button"
              class="articles-button-next opacity-20 hover:opacity-100 transition-all"
            >
              <svg
                width="22"
                height="24"
...
const pageData = computed(
  () =>
    itemData.value?.translations?.filter((item) => item.languages_code.includes(locale.value))[0],
);
...
useHead(() => ({
  ...
  meta: [
    ...
    {
      property: 'og:image',
      content: itemData.value?.background
        ? `https://api.avalonbali.com/assets/${itemData.value.background}`
        : '/img/og-image.jpg',
    },
    {
      property: 'og:url',
      content: `https://avalonbali.com/articles/${route.params.id}`,
    },
    { property: 'og:type', content: 'article' },
    { property: 'og:site_name', content: 'Avalon' },
    { property: 'article:published_time', content: itemData.value?.date_created },

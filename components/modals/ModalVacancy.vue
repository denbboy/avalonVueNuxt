    ...
    <form @submit.prevent="handleSubmitForm" class="relative z-[1]">
      ...
      >
        <input
          ...
          accept=".doc, .docx, .txt, .rtf, .odt, .pdf, .jpg, .png"
          type="file"
          class="hidden"
        />
      ...
      <textarea
        v-model="message"
        class="custom-input resize-none h-[94px] lg:h-[120px]"
        name=""
        id=""
        :placeholder="$t('input_textarea')"
      ></textarea>
  ...
  try {
    if (!fullName.value || !email.value || !message.value || !validateEmail(email.value)) {
      ...
      if (file.value) {
        ...
        fileSending = await fetch('https://api.avalonbali.com/files', {
          method: 'POST',
          body: formData,
        });
      }

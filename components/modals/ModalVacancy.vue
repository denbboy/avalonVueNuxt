        ...
        <form @submit.prevent="handleSubmitForm" class="relative z-[1]">
            ...
            <label
                ...
                <p class="flex items-center">
                    ...
                    <span v-if="!file" class="text-white text-xs lg:text-base font-bold lg:ml-2">Прикрепить файл с
                        резюме</span>
                    <span v-if="file" class="text-white text-xs lg:text-base font-bold lg:ml-2">{{ file.name }}</span>
                </p>
                <p v-if="!file" class="text-white/40 mt-2 text-xs lg:text-base"> .doc, .docx, .txt, .rtf, .odt, .pdf до
                    5 Мб.</p>
            </label>

<template>
  <section class="bali py-14 md:py-24 bg-blue-600 relative overflow-hidden">
    <div class="container">
      <div
        class="bali_con grid grid-cols-2 gap-x-2 gap-y-7 md:grid-cols-3 md:gap-y-12 md:gap-x-5"
        data-aos="fade-up"
      >
        <h2
          class="font-bold text-white text-[40px] md:text-[45px] lg:text-[56px] md:leading-[100%] col-start-1 col-end-3 md:col-end-1"
          v-html="
            itemStrings
              ?.filter((item) => item.id === 130)[0]
        ...
        <div class="border-b border-whiteOp-300 pb-5 md:pb-8">
          <span class="font-bold text-white text-3xl md:text-[32px] lg:text-[40px]">
            <svg
              class="w-[46px] md:w-[59px] lg:w-[73px] h-[38px] md:h-[42px]"
              width="69"
              height="29"
              viewBox="0 0 69 29"
              fill="none"
          ...
          <div
            class="text-white text-sm mt-2 md:text-sm lg:text-base md:mt-4"
            v-html="
              itemStrings
                ?.filter((item) => item.id === 132)[0]
                ?.String_id?.translations?.filter((item) =>
                  item.languages_code.code.includes(locale),
                )[0]?.description
            "
        ...
        <div class="border-b border-whiteOp-300 pb-5 md:pb-8">
          <span
            class="font-bold text-white text-3xl md:text-[32px] lg:text-[40px]"
            v-html="
              itemStrings
                ?.filter((item) => item.id === 133)[0]
                ?.String_id?.translations?.filter((item) =>
                  item.languages_code.code.includes(locale),
                )[0]?.title
            "
        ...
        <img
          src="/img/index/a-bali-decor.svg"
          loading="lazy"
          width="110"
...
watchEffect(() => {
  ...
  if (isProjectPage) {
    itemStrings.value =
      props.itemStrings ??
      (blocksStore.blocks?.filter((item) => item?.id === 30)[0]?.strings || []);
  } else {

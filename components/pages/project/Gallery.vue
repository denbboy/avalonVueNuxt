    ...
    <section class="gallery lg:py-14 py-5 relative overflow-hidden">
        ...
        <div class="container relative z-10">
            <div
                ...
                <client-only>
                    <a v-for="image in gallery?.slice(0, isShowFull ? 9999 : 7)" @click="handleOpenImage" :key="image?.directus_files_id"
            ...
            <a data-fancybox="gallery"
                :href="`https://avalon-panel.sonisapps.com/assets/${gallery[0].directus_files_id}`" type="button"
                class="font-bold text-white text-sm w-full max-w-[205px] mt-5 border border-white rounded-xl p-4 m-auto flex justify-center hover:text-blue-400 hover:bg-white transition-all md:mt-7 md:p-6 md:max-w-[286px] md:text-base">
                {{ $t('show_album') }}
            </a>
        </div>
...
const handleOpenImage = () => {
    setTimeout(() => {
        document.querySelectorAll('.f-thumbs__slide__img').forEach((item) => {
        item.src = item.getAttribute('data-lazy-data-src')
    })
    }, 100)
}

<template>
  <section class="relative" :class="isPlay && isPlayMobile ? 'z-[100]' : ''">
    <div class="lg:pb-10 pb-5 pt-36 lg:pt-[290px] relative">
      ...
      <div
        class="banner max-w-none bg-center absolute top-0 left-0 w-full h-screen overflow-hidden"
        :class="isPlay && isPlayMobile ? 'opacity-100 z-20' : 'opacity-80 -z-10'"
      >
        <NuxtImg
          v-if="itemData?.background"
        ...
        <NuxtImg
          v-else
          :src="getYoutubePreview().thumb(itemData?.video ?? '') ?? ''"
          class="absolute top-0 left-0 w-full h-[110vh] object-cover z-0 hidden md:block"
          alt="Image"
        />
      ...
      <div class="container">
        <div class="xl:flex xl:justify-between xl:gap-[20px]">
          <div class="w-full max-w-[950px]">
            <div
              :class="isPlay ? 'opacity-0' : 'opacity-100'"
            ...
            >
              ...
              >
                <img
                  ...
                  src="/img/icons/about-border-white.svg"
                  alt="bgd"
                  loading="lazy"
                />
              ...
              <div class="con order-1 md:order-none">
                <div
                  ...
                  v-if="itemData?.price_local && itemData?.price && itemData?.is_price_show"
                >
                  <img
                    class="absolute w-full h-full top-0 left-0 -z-10 md:hidden"
                    src="/img/index/bgd-decor.png"
                    alt="bgd"
                    loading="lazy"
                  />
                  <img
                    class="absolute top-0 left-0 -z-10 hidden md:block h-full w-full"
                    src="/img/index/bgd-decor-2.png"
                    alt="bgd"
                    loading="lazy"
                  />
              ...
              >
                ...
                >
                  <g clip-path="url(#clip0_840_7506)">
                    ...
                    />
                    <path
                      d="M26.8868 37.2038H19.6875V24.6454H21.9838V35.1488H26.8868V37.2038Z"
                      fill="white"
                      style="fill: white; fill-opacity: 1"
                    />
                    <path
                      d="M30.7318 37.2202H28.4355V24.662H30.7318V37.2202Z"
                      fill="white"
                      style="fill: white; fill-opacity: 1"
                    />
                    <path
            ...
            <div class="flex lg:gap-12 gap-6 items-center lg:mt-6 mt-5">
              ...
              <span
                class="text-white lg:text-[50px] text-[24px]"
            ...
            <div
              v-if="itemData?.translations?.length"
              :class="isPlay ? 'opacity-0' : 'opacity-100'"
              class="text-white text-sm max-w-64 md:max-w-[501px] md:text-base mt-3 md:mt-6"
              v-html="
                itemData?.translations?.find((item) => item.languages_code.includes(locale))
                  ?.description
              "
            ></div>
          ...
          <div class="flex flex-col xl:items-end justify-end xl:w-full">
            <button
              v-if="itemData?.video"
              @click="(e) => handlePlayVideo(itemData?.video, e)"
              type="button"
            ...
            >
              ...
              >
                ...
                <svg
                  v-if="!isPlay"
                  class="absolute -ml-5 lg:-ml-10 animate-scaling"
                  width="18"
                  height="21"
                  viewBox="0 0 18 21"
                  fill="none"
                ...
                >
                  <path
                    d="M17.7422 10.653L0.17076 20.7979L0.170761 0.508136L17.7422 10.653Z"
                    fill="white"
                  />
                </svg>
                <svg
                  v-else
                  id="blue_copy"
                  class="absolute -ml-5 w-1/4 lg:-ml-10 animate-scaling"
                  version="1.1"
                  viewBox="0 0 100 100"
                  xml:space="preserve"
                  xmlns="http://www.w3.org/2000/svg"
            ...
            >
              <div
                v-if="itemData?.villa_count && !$viewport.isLessThan('tablet')"
              ...
              >
                ...
                <p class="text-white text-sm md:text-base">
                  ...
                  {{
                    itemData?.translations?.filter((item) =>
                      item.languages_code.includes(locale),
                    )[0]?.first_counter ?? getVillaDeclension(itemData?.villa_count)
                  }}
                </p>
              </div>
              <div
                v-if="itemData?.roi_procent"
              ...
              >
                ...
                >
                  ...
                  <img
                    src="/img/icons/about-banner-A-ic.svg"
                    class="max-w-[20px] md:max-w-[30px] w-full"
                    alt="ic"
                    loading="lazy"
                  />
                </h3>
              ...
              <div
                v-if="itemData?.apartments_count"
              ...
              >
                ...
                <p class="text-white text-sm md:text-base">
                  ...
                  {{
                    itemData?.translations?.filter((item) =>
                      item.languages_code.includes(locale),
                    )[0]?.second_counter ?? getAppartmentDeclension(itemData?.apartments_count)
                  }}
                </p>
              </div>
            </div>
...
const handlePlayVideo = (videoUrl, e) => {
  ...
  const urlLink = `https://www.youtube.com/embed/${
    params?.query?.v ?? params?.pathname?.replace('/embed', '')?.replace('/', '')
  }?autoplay=1&mute=1&vq=hd720&loop=1&rel=0&modestbranding=1&fs=0&controls=0&playlist=${
    params?.query?.v ?? params?.pathname?.replace('/embed', '')?.replace('/', '')
  }`;

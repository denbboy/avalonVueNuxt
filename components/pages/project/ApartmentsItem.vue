<template>
  <!-- data-aos="fade-up" -->
  <div
    :class="props.itemData?.roi ? 'pt-24' : 'pt-5'"
  ...
  >
    <div
      class="bg-blue-600/85 w-full h-full z-0 rounded-2xl backdrop-blur-sm absolute left-0 top-0"
    ></div>

    <div
      v-if="props.itemData?.roi"
    ...
    >
      ...
      ></div>
      <p class="text-white text-xs 3xl:text-lg md:uppercase">Roi</p>
    ...
    <div class="relative w-full h-full flex justify-end">
      ...
      >
        ...
        ></div>
        <p class="text-white text-xs 3xl:text-lg md:uppercase">Roi</p>
        <span class="text-white font-bold text-lg 3xl:text-3xl"> {{ props.itemData?.roi }}% </span>
      </div>
      ...
      >
        ...
        <client-only>
          <swiper
            class="swiper-apartments w-full h-full"
            :modules="modules"
            :slides-per-view="1"
          ...
          >
            <swiper-slide
              v-for="image in props.itemData?.gallery"
              :key="image?.id"
            ...
            >
              ...
              >
                <NuxtImg
                  ...
                  class="rounded-[15px] object-cover h-full w-full"
                  alt="ph"
                  loading="lazy"
                />
              </a>
...
translations.value =
  props.itemData?.translations?.find((item) => item.languages_code.includes(locale.value)) || {};

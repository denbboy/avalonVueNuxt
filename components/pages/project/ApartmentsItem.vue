    ...
    <div :class="itemData?.roi ? 'pt-24' : 'pt-5'"
        ...
        <div class="relative w-full flex justify-end h-full aspect-[5/4]">
            ...
            <div
                ...
                <client-only>
                    <swiper class="swiper-apartments w-full h-full" :modules="modules" :slides-per-view="1"
                        ...
                        <swiper-slide v-for="image in itemData?.gallery" :key="image?.id"
                            ...
                            <a :href="`https://avalon-panel.sonisapps.com/assets/${image?.directus_files_id}?width=600&height=500`" :data-fancybox="itemData?.id">
                                <NuxtImg :src="`https://avalon-panel.sonisapps.com/assets/${image?.directus_files_id}?width=600&height=500`"
                ...
                <div :class="'swiper-button-prev after:hidden prev-btn-' + itemData?.id">
                    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const navigationConfig = ref({
    nextEl: '.next-btn-' + itemData?.id,
    prevEl: '.prev-btn-' + itemData?.id,
});
</script>

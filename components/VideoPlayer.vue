<template>
  <div class="bg-center absolute top-0 left-0 w-full h-[100%] -z-10 opacity-50">
    <div v-if="!item?.video">
      <NuxtImg
        ...
        class="absolute brightness-[.5] top-0 left-0 w-full h-full change"
        alt="Image"
      />
      <NuxtImg
        v-else
        :src="`https://api.avalonbali.com/assets/${item?.img}?width=1280&height=600&format=webp`"
        class="absolute brightness-[.5] top-0 left-0 w-full h-full change"
        alt="Image"
      />
    </div>
    ...
    <div v-if="item?.video" class="h-full w-full">
      <NuxtImg
        ...
        width="1920"
        height="800"
        class="absolute hidden md:block object-cover brightness-[.5] top-0 left-0 w-full h-full"
        alt="Image"
      />
      ...
      <iframe
        loading="lazy"
        :class="isVideoPlayed ? 'full-video' : ''"
        class="lg:scale-150 scale-[3] pointer-events-none w-full h-full"
        title="Avalon"
        frameborder="0"
  ...
  <div
    class="bg-gradient-to-t from-blue-500 from-30% w-full h-52 absolute -z-10 bottom-0 left-0"
  ></div>
  <div
    class="bg-gradient-to-t from-blue-500 from-30% w-full h-52 absolute -z-10 top-0 left-0 rotate-180"
  ></div>
  ...
  <div class="container">
    <div class="flex md:flex-row flex-col items-center" v-if="item?.projects?.length">
      <div class="is-will-hidden w-full min-h-[200px]">
        ...
        >
          <NuxtImg
            v-if="item?.projects[0]?.item.logo"
            width="100"
            height="95"
          ...
          <div class="flex items-center w-full gap-5">
            ...
            <div
              class="relative px-5 py-3 w-fit md:text-center md:px-7"
              v-if="
            ...
            >
              ...
              <img
                v-else
                class="absolute top-0 left-0 w-full -z-10 hidden md:block h-full"
        ...
        <div class="flex items-center justify-between md:block md:mt-7 mt-3 lg:mt-12">
          ...
          <button
            data-aos="fade-up"
            v-if="item?.video"
            @click="(e) => handlePlayVideo(e, item?.video)"
            type="button"
          ...
          >
            <div
              class="relative flex items-center justify-center max-w-[95px] w-full xl:max-w-[165px]"
            >
              <svg
                v-if="!isVideoPlayed"
            ...
            <span :class="isVideoPlayed ? 'opacity-0 invisible' : 'opacity-100 visible'">{{
              $t('see_video')
            }}</span>
          </button>
        </div>
...
const handlePlayVideo = (e, videoUrl) => {
  ...
  const videoId =
    params.searchParams.get('v') || params.pathname.replace('/embed', '').replace('/', '');

<template>
    <NuxtLink :href="`/sales/${props?.item?.id}`" class="group h-full block relative rounded-2xl transition-all">
        ...
        <SkeletonLoader class="w-full h-[300px] rounded-2xl">
            <img v-show="!imageLoaded" data-not-lazy loading="lazy" ref="image" class="absolute opacity-0 w-full h-full"
                :src="`https://avalon-panel.sonisapps.com/assets/${item?.preview}`" @load="onImageLoad" />
            <img data-not-lazy loading="lazy" v-if="imageLoaded"
        ...
        <div class="absolute z-20 w-full h-full top-0 left-0 p-3 flex flex-col">
            <img src="/assets/img/icons/icon-club.svg" alt="Image"

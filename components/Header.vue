<template>
    ...
    class=" z-30 transition-all absolute w-full">
      ...
      :class="isOpenBurger ? 'bg-blue-500' : 'md:bg-transparent'">
        ...
        <div class="header__inner px-5 flex items-center justify-between gap-3 relative z-[2]">
          ...
          <a :href="`https://wa.me/${toolkitStore?.settings?.phone?.replace(/[^\d+]/g, '')}`" class="xl:flex hidden items-center gap-1 text-white md:text-sm lg:text-base font-bold ">
          ...
          <div class="hidden group xl:flex xl:flex-col relative">
            ...
            <div
              ...
              <button v-if="locale !== 'ua'" @click="changeLocale('ua')" class="text-white md:text-xs lg:text-sm mb-4 last:mb-0 hover:text-blue-400 transition-all">
                UA
              </button>
              <button v-if="locale !== 'ru'" @click="changeLocale('ru')" class="text-white md:text-xs lg:text-sm mb-4 last:mb-0 hover:text-blue-400 transition-all">
                RU
              </button>
              <button v-if="locale !== 'en'" @click="changeLocale('en')" class="text-white md:text-xs lg:text-sm mb-4 last:mb-0 hover:text-blue-400 transition-all">
                EN
              </button>
            </div>
          </div>
  ...
  <div id="burgerMenu"
        ...
        <div class=" flex flex-col gap-4">
          <a :href="`tel:${toolkitStore?.settings?.phone}`" class="hover:text-blue-400 transition-all font-bold text-white text-base">
            {{ toolkitStore?.settings?.phone }}
          </a>
